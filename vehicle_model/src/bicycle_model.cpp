...
public:
    BicycleModelNode() : Node("bicycle_model_node") {
        ...
        orientation_pub_ = create_publisher<std_msgs::msg::Float64>("orientation", 10);
        v_position_pub_ = create_publisher<std_msgs::msg::Float64>("velocity", 10);
        x_position_pub_ = create_publisher<std_msgs::msg::Float64>("x_position", 10);
        y_position_pub_ = create_publisher<std_msgs::msg::Float64>("y_position", 10);

        // Initialize subscribers for acceleration and steering rate
        acceleration_sub_ = create_subscription<std_msgs::msg::Float64>(
            "acceleration", 10, [this](const std_msgs::msg::Float64::SharedPtr msg) {
                // Update acceleration value
                acceleration_ = msg->data;
            });

        steering_rate_sub_ = create_subscription<std_msgs::msg::Float64>(
            "steering_rate", 10, [this](const std_msgs::msg::Float64::SharedPtr msg) {
...
private:
    ...
    rclcpp::Publisher<std_msgs::msg::Float64>::SharedPtr v_position_pub_;
    ...
    void simulateBicycleModel() {
        ...
        orientation_ += (velocity_ / length) * tan(steering_rate_) * dt;
        x_position_ += velocity_ * cos(orientation_) * dt;
        y_position_ += velocity_ * sin(orientation_) * dt;


        RCLCPP_INFO(this->get_logger(), "Velo: '%f'", velocity_);
